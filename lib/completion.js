...
Completion.prototype = {
  ...
  completeInfo: function (info, cb) {
    ...
    // Otherwise, attempt to keep on recursing
      ...
      // TODO: This is fucked. Everything is fucked.
      // Previously, we were recursing on the next node without knowing its parent context
      // Ideally, we should be recursing on the current latest match of the command
      ...
      var matchedCommandNode = commandNodes.filter(function matchCommand (commandNode) {
        // TODO: It looks like we are one too many ahead/behind. Figure out why and how to fix it.
        console.log(commandNode.name, nextWord);
        return commandNode.name === nextWord;
      })[0];
      console.log(matchedCommandNode);
