...
// TODO: Update documentation for `Completion's parentNode`
// TODO: Document `completeInfo`, and `matchLeftWord`
...
Completion._optionalKey = '_completionOptional';
// Mark a function as optional
Completion.optional = function (fn) {
  fn[Completion._optionalKey] = true;
  return fn;
};
Completion.prototype = {
  ...
  completeInfo: function (info, cb) {
    ...
    // The following is resolves the completion via either:
    // (1) terminating at a function that is an option which needs to recurse further
      // `git -b checkout he|llo` -> ['git', '-b', 'checkout', 'he'] + ['llo'] -> git['-b'] -> git.checkout('he')
    // (2) a recursive loop that creates child completion's until we arrive
    ...
    // FIRST PATH (option logic)
    if (typeof node === 'function' && node[Completion._optionalKey] === true) {
      return node.call(this, info, cb);
    }

    // SECOND PATH (recursive loop)
